...
namespace BitTagUser.CutomerServices
{
    public class BitTags:IBitTag
    {
		...
		public async Task<List<BitTagDetailsModel>> GetBitTByID(string qr)
		{
			List<BitTagDetailsModel> bitTagDetails = new List<BitTagDetailsModel>();
			bitTagDetails = await _httpClient.GetFromJsonAsync<List<BitTagDetailsModel>>("api/controller/GetBitTagDetailsById/"+qr);
			if (bitTagDetails != null)
			{
				return bitTagDetails;
			}
